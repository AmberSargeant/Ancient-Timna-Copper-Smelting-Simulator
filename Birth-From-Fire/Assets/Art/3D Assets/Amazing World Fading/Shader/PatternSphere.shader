Shader "Amazing World Fading/Pattern Sphere" {
	Properties {
		...
		[HDR]_BorderColor      ("Border Color", Color) = (0.86, 0.2, 0.2, 0)
	...
	SubShader {
		...
		sampler2D _Albedo, _Normal, _Metallic;
		float4 _BorderColor;
		float3 _Pos, _NoiseSpeed;
		float _Radius, _Invert, _BorderRadius, _BorderNoiseScale, _Cutoff, _Smoothness;

		void surf (Input input, inout SurfaceOutputStandard o)
		{
			float2 uv = input.uv_Albedo;
			o.Normal = UnpackNormal(tex2D(_Normal, uv));
			o.Albedo = tex2D(_Albedo, uv).rgb;
