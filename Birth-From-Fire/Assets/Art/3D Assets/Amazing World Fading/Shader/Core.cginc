#ifndef AWD_CORE_INCLUDED
#define AWD_CORE_INCLUDED

#include "UnityCG.cginc"

struct Input
{
	float2 uv_MainTex;
	float2 uv_NoiseTex;
	float2 uv_BumpMap;
	float2 uv_SecondaryTex;
	float3 worldPos;
	float3 stripeUvw;
};

sampler2D _MainTex, _BumpMap, _SecondaryTex;
float3 _Pos, _FadeColor;
float  _VisDist, _FadeWidth, _FadeIntensity, _Glossiness, _Metallic;

float Fade (float3 wldpos)
{
#ifdef AWD_SPHERICAL
	float dist = distance(wldpos, _Pos);
	float f = smoothstep(_VisDist, _VisDist + _FadeWidth, dist);
#endif
#ifdef AWD_CUBIC
	float distX = distance(wldpos.x, _Pos.x);
	float distY = distance(wldpos.y, _Pos.y);
	float distZ = distance(wldpos.z, _Pos.z);
	float fadeX = smoothstep(_VisDist, _VisDist + _FadeWidth, distX);
	float fadeY = smoothstep(_VisDist, _VisDist + _FadeWidth, distY);
	float fadeZ = smoothstep(_VisDist, _VisDist + _FadeWidth, distZ);
	float f = max(fadeX, fadeZ);
	f = max(f, fadeY);
#endif
#ifdef AWD_ROUND_XZ
	float dist = distance(wldpos.xz, _Pos.xz);
	float f = smoothstep(_VisDist, _VisDist + _FadeWidth, dist);
#endif
	return f;
}

//float3 _ConePos, _ConeNormal;
//float _ConeHeight, _ConeRadius;
//float FadeCone (float3 wldpos, float noise)
//{
//	float3 p1 = _ConePos;
//	float3 p2 = p1 + normalize(_ConeNormal) * _ConeHeight;
//	float t = max(0, min(1, dot(wldpos - p1, p2 - p1) / (_ConeHeight * _ConeHeight)));
//	float3 proj = p1 + t * (p2 - p1);
//	float d = distance(wldpos, proj);

//	float radius = lerp(0, _ConeRadius, t);
//	radius -= noise;
////float cutout = max(0, d - radius);
//	float cutout = radius - min(d, radius);
//	return (cutout > 0 ? cutout : -1);
//}

float PseudoRandom (float2 v)
{
	return frac(sin(dot(v, float2(12.9898, 78.233))) * 43758.5453123);
}
#endif